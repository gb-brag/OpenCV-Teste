...
int main(void) {
	...
	cvtColor(image, cimage, 6);
	cimage.convertTo(cimage, CV_8UC1);

	cout << "1> Media = " << getAverage<Vec3b>(cimage) << endl;
	cout << "1> Media = " << getAverage<uchar>(cimage) << endl;

	cout << "2> Media = " << getAverage<Vec3b>(cimage) << endl;
	cout << "2> Media = " << getAverage<uchar>(cimage) << endl;

	cout << "Desvio padrao = " << getStdDeviation<Vec3b>(cimage) << endl;
	...
	// Create a window for display.
	namedWindow("cinza", WINDOW_NORMAL);
	// Show our image inside it.
	imshow("cinza", cimage);
